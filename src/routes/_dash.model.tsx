...
const MAX_SERBIAN_CONSUMPTION_MCM = 19;
...
export function buildAnalysis(opts: {
  ...
  return opts.dates.map((date): AnalysisRow => {
    ...
    serbianConsumption = Math.min(MAX_SERBIAN_CONSUMPTION_MCM, Math.max(0, serbianConsumption));
    ...
    return {
      ...
      serbian_consumption_mcm: serbianConsumption,
      temperature_c,
...
// day's value. An "extreme" is > 2.5Ã— or < 0.4Ã— the previous non-zero value.
// Used for chart display only â€” raw analysis rows are preserved for tables.
export function smoothExtremes<T extends object>(rows: T[], fields: (keyof T)[]): T[] {
...
export function reconstructPrice(opts: {
  ...
  fxByMonth: Record<string, number>; // EUR per USD or USD per EUR? â€” assume EUR/USD i.e. 1 EUR = X USD
...
// Realistic monthly approximations for EU benchmarks 2020â€“2026.
// User can override these via CSV upload.
export function syntheticTtf(month: string): number {
  // EUR/MWh â€” calibrated approximation of monthly TTF Day-Ahead averages.
